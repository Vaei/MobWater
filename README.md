...
- **Waterfalls, on their own terms.** A spline for the lip, per point drops, and a sheet drawn by how long ago the water at a pixel left the edge - so the pattern rides down with the parcel it belongs to, stretches as the water accelerates away from itself, thins because the same water passing faster has to be thinner, and comes apart into strands where it has been drawn out furthest. Not a shape of the surface: every meaning a body of water attaches to its geometry is a plan view, and none of it survives being stood on end
- **The two joins are the work, and both are done.** The lip reads the water feeding it at each end of its line and moves onto it, so a swell does not detach the fall from the river on every wave; the base leaves a persistent disturbance in the ripple field, so the pool below never settles. A fall registers no body of water, because nothing floats on one
