...
public:
	...
	/**
	 * How far the water falls at each point along the lip, in world units.
	 *
	 * Per point, so a fall over an uneven ledge reaches a plunge pool that is not level. Short lists
	 * are padded with the last entry rather than treated as an error, for the same reason a river's
	 * widths are: a spline gains points by being dragged, and refusing to build until someone tops
	 * the array up would mean the fall vanishes mid-edit.
	 */
	...
	/** The drop at a distance along the lip, padded and interpolated from Drops. */
